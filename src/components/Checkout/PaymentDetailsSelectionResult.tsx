...
export default function PaymentDetailsSelectionResult() {
    ...
    return watch().selected === 1 ? <div className="flex gap-[16px] max-sm:flex-col">
    ...
    </div> : watch().selected === 2 ? <div className="flex gap-[32px] max-sm:flex-col items-center">
        <img src="/images/checkout/icon-cash-on-delivery.svg" alt="" />
        <p className={`${P} text-[rgba(0,0,0,0.5)]! max-sm:text-center`}>The ‘Cash on Delivery’ option enables you to pay in cash when our delivery courier arrives at your residence. Just make sure your address is correct so that your order will not be cancelled.</p>
    </div> : undefined
